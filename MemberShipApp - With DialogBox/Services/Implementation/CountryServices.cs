...
namespace MemberShipApp.Services
{
    public class CountryServices : ICountryServices
    {
        ...
        async Task<IEnumerable<CountryDto>> ICountryServices.GetAllCountries()
        {
            var countries= await _unitOfWork.Countries.GetAllAsync();
            return countries.Select(s => DBFacade.CountryDto(s));
        }

        async Task<IEnumerable<CountryTuplate>> ICountryServices.GetAllCountryWithConnect()
        {
           var countries= await _unitOfWork.Countries.GetAllWithRegionsAndStatesAsync();
           return countries.Select(s => DBFacade.CountryTuplate(s));
        }
